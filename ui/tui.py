...
# Logo as Rich renderables — no fixed-width chars, so no overflow or stray lines
def _om_den_logo() -> Group:
    title = Text()
    title.append("  ", style="dim")
    title.append("O", style="bold bright_white")
    title.append("m", style="bold gold1")
    title.append("'s ", style="bold gold1")
    title.append("D", style="bold bright_white")
    title.append("en", style="bold gold1")
    tagline = Text("  where code meets clarity  ", style="dim italic")
    return Group(
        Text(""),
        Align.center(Text("✦", style="dim")),
        Align.center(title),
...
def _assistant_sep() -> Text:
    """Short separator for assistant block — no full-width Rule to avoid stray lines."""
    t = Text("  ", style="dim")
    t.append("· ", style="dim")
    t.append("· ", style="dim")
    t.append("Om", style="assistant")
    t.append(" · ", style="dim")
    t.append("·", style="dim")
    return t
...
class TUI:
    ...
    def begin_assistant(self) -> None:
        self.console.print()
        self.console.print(_assistant_sep())
    ...
    def print_welcome(self, title: str, lines: list[str]) -> None:
        self.console.print()
        # Logo in a compact panel so nothing overflows
        self.console.print(
            Panel(
                _om_den_logo(),
                border_style="dim",
        ...
        info_table.add_column(style="muted", justify="right", no_wrap=True)
        info_table.add_column(style="code", overflow="fold")
        for line in lines:
            if ": " in line:
                label, value = line.split(": ", 1)
                info_table.add_row(label.strip(), value.strip())
            else:
        ...
        self.console.print()
        self.console.print(
            Text("  ", style="dim") + Text("›", style="muted") + Text(" Type a message or ", style="dim") + Text("/help", style="info") + Text(" for commands.", style="dim")
        )
        self.console.print()
...
**Tips** — Type your message to chat. Om can read, write, and run code. Some actions need approval.
"""),
            title=Text("  Om's Den ", style="highlight") + Text("· Help", style="dim"),
            title_align="left",
            border_style="border",
            box=box.ROUNDED,
            padding=(1, 2),
