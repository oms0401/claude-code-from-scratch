...
class Config(BaseModel):
    ...
    # OpenRouter URL — used when API key looks like OpenRouter (sk-or-v1-...)
    OPENROUTER_BASE_URL: ClassVar[str] = "https://openrouter.ai/api/v1"
    ...
    def base_url(self) -> str | None:
        ...
        if url:
            return url
        # OpenRouter keys start with sk-or-; use OpenRouter URL so the key is not sent to OpenAI
        key = self.api_key or ""
        if key.strip().startswith("sk-or-"):
